...
function initSound()
{
	...
	if(isIpad){
		document.getElementById('maiDiv').style.marginLeft= '10%';
	}
	getxmlValue();
	pageUpdate = 'no';
}
...
function getxmlValue()
{
	...
	if(par.length==1 || (par.length == 0 && isIe))
	{
...
function loadAudioSound()
{
	...
	$("#jquery_jplayer").jPlayer({
		ready: function () {
			...
			this.element.jPlayer("setFile", "audio/pan.mp3", "audio/pan.ogg");
 ...
 if(loadPercent <= 100 && !audioLoaded){
				...
				if(loadPercent == 100){
					...
					if(!isIpad && !chromeBrowser){
						...
						$("#jquery_jplayer").jPlayer("playHeadTime",(32 * 1000));
					}
...
function clearPrevSpan(spanId){
	...
	document.getElementById(currentAudioPage).contentWindow.document.getElementById(spanId).style.backgroundColor="#fff"; 
}
