...
function TiledMaze(){
	...
	this.addImages = function(){
		...
		_rat = repo.getRat();
		_rat.x = 50;
		_rat.y = (MAZE_HEIGHT-1)*TILE_SIZE +40;
		_rat.pos().x = 50;
		_rat.pos().y = (MAZE_HEIGHT-1)*TILE_SIZE +40;
	...
	function howToPlayClicked(event){
		var snd = new Audio("Assets/audio/CG-ManInYellowHat_BeanCreative_A-MAZE-ingAdventure_Introduction_VO_1_12"+SUFFIX);
	...
	function letterClickHandler(event){
		...
		if (this.decoy){
			...
			if (Math.random()>Math.random())
				snd = new Audio("Assets/audio/CG-ManInYellowHat_BeanCreative_A-MAZE-ingAdventure_Feedback_VO_1_22-2"+SUFFIX);
			else
				snd = new Audio("Assets/audio/CG-ManInYellowHat_BeanCreative_A-MAZE-ingAdventure_Feedback_VO_1_24-1"+SUFFIX);
		...
		if (this == _currentLetter){
			...
			if (_reactLetters.indexOf(this.character)!=-1)
				alert ("You found the letter "+this.character+"! \n There will be art and audio here");
			_container.removeChild(_currentLetter);
			if (this.nextIndex != -1)
...
(function (window){
	function Rat(image){
		...
		this.onTick = function(){
			if ((Math.abs(pos.x-this.x) >= 2 ||Math.abs(pos.y-this.y) >= 2)&& !this.moving){
				this.moving = true;
