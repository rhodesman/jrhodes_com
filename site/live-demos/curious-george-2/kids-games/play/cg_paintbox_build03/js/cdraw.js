...
cdraw.setupHelpButton = function() {
  $.cookies.set('html_player', 1, {expiresAt: new Date((new Date()).getTime() + 30*24*60*60*1000)});
...
cdraw.openDoneDialog = function() {
  cdraw.audio.playHint('what_do_you_want_to_do_next', 0);
  cdraw.crt_tool_unload();
  $('#cgp-donedlg').show();
...
//    this.setupHint('#cgp-tool-brush', 'tool_brush', true);
//    this.setupHint('#cgp-tool-eraser', 'tool_eraser', true);
...
//    this.setupHint('#cgp-tool-stamp', 'tool_stamp');
    this.setupHint('#cgp-btn-done', 'btn_done');
//    this.setupHint('#cgp-clipart', 'choose_a_picture');
//    this.setupHint('#cgp-colorpicker', 'choose_a_color');
    this.setupHint('#cgp-background-options', 'background_choose_paper');
//    this.setupHint('.size-selector-sample-small', 'size_small');
//    this.setupHint('.size-selector-sample-medium', 'size_medium');
//    this.setupHint('.size-selector-sample-big', 'size_big');
    this.setupHint('#cgp-donedlg-btn-startover', 'start_over');
    this.setupHint('#cgp-donedlg-btn-close, #cgp-donedlg-btn-keeppainting', 'keep_drawing');
    this.setupHint('#cgp-donedlg-btn-print, #cgp-donedlg-btn-download', 'save_and_print');
  ...
  stopCurrentHint: function() {
    try {
      if (this.currenlty_playing_tag_id) {
...
cdraw.background = {
  ...
  setNextFrame: function() {
    ...
    this.display_frame++;
    if (this.display_frame > cgp_conf.background_tool.frames_count) {
  ...
  setupFrame: function() {
    ...
    if (cdraw.background.display_frame == 0) {
      cdraw.frame_layer.removeChildren();
      view.draw();
      return;
    }
