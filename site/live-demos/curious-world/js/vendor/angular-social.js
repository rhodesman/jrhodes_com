...
app.filter('shorterRound', function() {
        ...
        return {
            ...
            controller: ['$scope', '$q', '$http', function ($scope, $q, $http) {
                ...
                var ctrl = {
                    ...
                    facebookShare: function(e,options){
                        FB.ui(
                        {
                            method: 'feed',
                            name: $scope.title,
...
app.directive('ngSocialFacebook', function() {
    ...
    var options = {
        ...
        popup: {
            url: 'https://www.facebook.com/sharer/sharer.php?s=100&p[url]={url}&p[images][0]={image}&p[title]={title}&p[summary]={description}',
            width: 600,
            height: 500
        },
        track: {
    ...
    return {
        ...
        transclude: true,
        template: '<li>' +
                    '<a ng-href="{{ctrl.link(options)}}" target="_blank" ng-click="ctrl.facebookShare($event, options)"' +
                        ' class="ng-social-button">' +
                        '<span class="ng-social-icon"></span>' +
                        '<span class="ng-social-text" ng-transclude></span>' +
                    '</a>' +
                    '<span ng-show="count" class="ng-social-counter">{{ count | shorterRound }}</span>' +
                   '</li>',
...
app.directive('ngSocialTwitter', function() {
    ...
    var options = {
        ...
        popup: {
            url: 'https://twitter.com/intent/tweet?url={url}&text={title} - {image}',
            width: 600,
            height: 450
        },
...
app.directive('ngSocialVk', function() {
    ...
    var options = {
        counter:{
            url: 'https://vkontakte.ru/share.php?act=count&url={url}&index={index}',
            get: function(jsonUrl, deferred, $http) {
                if (!options._) {
                    options._ = [];
...
angular.module("ngSocial").directive('ngSocialOdnoklassniki', function() {
    var options = {
        counter: {
            url: 'https://www.odnoklassniki.ru/dk?st.cmd=shareData&ref={url}&cb=JSON_CALLBACK',
            getNumber: function(data) {
                return data.count;
            }
        },
        popup: {
            url: 'https://www.odnoklassniki.ru/dk?st.cmd=addShare&st._surl={url}',
            width: 550,
            height: 360
        },
        track: {
...
angular.module("ngSocial").directive('ngSocialMailru', function() {
    var options = {
        counter: {
            url: 'https://connect.mail.ru/share_count?url_list={url}&callback=1&func=JSON_CALLBACK',
            getNumber: function(data) {
