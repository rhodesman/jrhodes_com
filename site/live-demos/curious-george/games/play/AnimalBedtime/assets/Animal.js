...
Animal.prototype.Container_initialize = Animal.prototype.initialize;
    Animal.prototype.initialize = function() {
        ...
        this.regX = bmp.width/2;
        this.regY = bmp.height/2;
        //this.addChild(bmp);
        this.addChild(this.blinkAnimation);
        if (this.tailAnimation != null){
             //this.addChild(this.tailAnimation);
        }
        var g = new createjs.Graphics().beginFill("#FFF").drawRect(0,0,100,100);//this.sitBMP.width, this.sitBMP.height);
        var s = new createjs.Shape(g);
        s.alpha = 0.02;
        this.addChild(s);
        //this.sitBMP.hitArea = new createjs.Shape(g);
    ...
    /*Animal.prototype.dr
    Animal.prototype.draw = function(ctx, ignoreCache) {
			
			// update hit area so the full text area is clickable, not just the characters:
            this.draw(ctx,ignoreCache);
			this.hitArea.graphics.clear().beginFill("#FFF").drawRect(0,0,100,100);//this.lineWidth||this.getMeasuredWidth(), this.getMeasuredHeight());
		}*/
