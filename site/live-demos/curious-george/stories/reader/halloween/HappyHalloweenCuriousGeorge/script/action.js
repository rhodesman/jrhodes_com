...
/*function onTimeUpdate(loadPercent, playedPercentRelative, playedPercentAbsolute, playedTime, totalTime){
 console.log("Total Time-->"+(totalTime)/1000+"Cuurent Time-->"+currentTime)
    if( (currentTime >= 28.5 || (currentTime >= 25.8 && chromeBrowser)) && currentPlayer == 'jquery_jplayer_1' )	
	{
		console.log("player 2 set");
	   $('#'+currentPlayer).jPlayer('pause');
	   currentPlayer = 'jquery_jplayer_2';
	   if(chromeBrowser)
	   		$('#'+currentPlayer).jPlayer("playHeadTime",(25.2 * 1000));
	   else
	   		$('#'+currentPlayer).jPlayer("playHeadTime",(28.7 * 1000));
	   $('#'+currentPlayer).jPlayer("onProgressChange",onTimeUpdate);
	}
	/*else if(currentTime < 28.5 && currentPlayer == 'jquery_jplayer_2'){
		console.log("player 1 set");
		$('#'+currentPlayer).jPlayer('pause');
	   currentPlayer = 'jquery_jplayer_1';
	   $('#'+currentPlayer).jPlayer("playHeadTime",(22 * 1000));
	   $('#'+currentPlayer).jPlayer("onProgressChange",onTimeUpdate);
	}
				   ...
				   if(loadPercent == 100  || isIpad)
					{
						...
						audioLoaded = true;
						$("#"+currentPlayer).jPlayer("volume",50);
						if(!isIpad){
							playAudio('playBtn');
						}
					}
...
/*function nextPageAction()
{
	...
	{
	$("#"+currentPlayer).jPlayer("pause");
	$("#"+currentPlayer).jPlayer("play",(Number(sValue) * 1000));	
	}
}*/
...
{ 
	...
	{
	   currentPlayer = 'jquery_jplayer_2';
	   $('#'+currentPlayer).jPlayer("playHeadTime",(Number(sValue) * 1000));
	   $('#'+currentPlayer).jPlayer("onProgressChange",onTimeUpdate);
	   return;
	}
	else{
