...
function loadAudioSound()
{
	...
	$("#jquery_jplayer").jPlayer({
		ready: function () {
			...
			this.element.jPlayer("setFile", "audio/pan2.mp3", "audio/pan2.ogg");
 ...
 if(loadPercent <= 100 && !audioLoaded){
				...
				if(loadPercent == 100 || isIpad){
					...
					if(!isIpad && !chromeBrowser){
						...
						$("#jquery_jplayer").jPlayer("playHeadTime",(22* 1000));
					}
...
{ 
	if(Number(sValue)<1)
	{
	$("#jquery_jplayer").jPlayer("playHeadTime",(Number(sValue)+22 * 1000));	
	}
	else
	{
	$("#jquery_jplayer").jPlayer("playHeadTime",(Number(sValue) * 1000));
