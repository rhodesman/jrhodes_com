...
if(isIpad)
{
	if(pageCount<3)
				{
					//document.getElementById('outerLine').style.opacity=0.2;
...
function initSound()
{
	...
	//	alert("hi ipad")
		...
		/*document.getElementById('playBtn').style.height="100px";
		document.getElementById('playBtn').style.width="100px";
			document.getElementById('playBtn').style.backgroundColor="red";
		document.getElementById('playBtn').style.marginLeft="-340px";
			document.getElementById('playBtn').style.marginTop="-280px";
				document.getElementById('playBtn').style.position="absolute";*/
					//audioPlayed=true;
					//document.getElementById('outerLine').style.opacity=1;
				
				
		

		//alert(document.getElementById('PlayIpad').innerHTML)
	}
...
function loadAudioSound()
{
	...
	$("#jquery_jplayer").jPlayer({
		ready: function () {
			...
			this.element.jPlayer("setFile", "audio/CG GOES TO THE ZOO.mp3", "audio/CG GOES TO THE ZOO.ogg");
			//this.element.jPlayer("setFile", "http://newmedia.laserwords.com/test/audio/pan.mp3", "http://newmedia.laserwords.com/test/audio/pan.ogg");
 ...
 if(loadPercent <= 100 && !audioLoaded){
				...
				if(loadPercent == 100 || isIpad){
					...
					if(!isIpad && !chromeBrowser){
						...
						$("#jquery_jplayer").jPlayer("playHeadTime",(0* 1000));
					}
...
function clearPrevSpan(spanId){
	//alert(sValue)
	document.getElementById(currentAudioPage).contentWindow.document.getElementById(spanId).style.color="#000";
