...
var vo_load_merged = "res/audio/vo/load_vo_merged_2";
...
if (iOS) {
  ...
  //generating the sound mix variables for ios
  var thestring = 'eghlmp';
  var thechars = thestring.split('');
  var combinations = [];
  var currentString = '';
  var currentOptions = thechars;
  var aStep = function(currentString, currentOptions){
    var currentString = currentString;
    var currentOptions = currentOptions;
    for(var i=0; i<currentOptions.length; i++){
      if(currentString != ''){
        if(  thestring.indexOf(  currentString.charAt( currentString.length-1 )) > thestring.indexOf( currentOptions[i] )  )
        continue;
      }
      var newCurrentString = currentString+currentOptions[i];
      combinations.push(newCurrentString);
      var newCurrentOptions = currentOptions.slice();
      newCurrentOptions.splice(i, 1);
      aStep(newCurrentString, newCurrentOptions);
      
    }
  }
  aStep(currentString, thechars); //init
  
  //including all the possible mixes
  for(var i=0; i<combinations.length; i++){
    window['snd_'+combinations[i]] = "res/audio/mixes/mix_" +  combinations[i];
    iOS_resources.push( {type:"effect", src:window['snd_'+combinations[i]]} );
  }
