...
cdraw.theme = {
  ...
  setupThemes: function() {
    var wait_for_themes_data_interval = window.setInterval(function() {
      if (typeof themes_data == 'undefined') {
        return;
      }
      window.clearInterval(wait_for_themes_data_interval);
      
      cdraw.theme.data_loaded = true;
      cdraw.theme.data = themes_data;
  ...
  setup: function() {
    ...
    $('#cgp-btn-theme-browser').click(function() {
      for (var i=0; i<cdraw.theme.data.themes_names.length; i++) {
        if (cdraw.theme.data.themes_names[i] == cdraw.theme.data.current_theme) {
          ...
          /* automatically activate the clipart tool */
          if (cdraw.crt_tool != cdraw.clipart_tool) {
            $('#cgp-tool-clipart').click();
          }
          break;
        }
      }
    ...
    var matches = cgp_conf.theme_url_exp.exec(top.location.href);
    if (matches) {
      var matched_theme = matches[1];
      for (var i=0; i<cdraw.theme.data.themes_names.length; i++) {
        if (cdraw.theme.data.themes_names[i].toLowerCase() == matched_theme.toLowerCase()) {
          cdraw.theme.data.current_theme = cdraw.theme.data.themes_names[i];
          break;
        }
      }
    }
    ...
    if (typeof cdraw.theme.data.themes[ cdraw.theme.data.current_theme ].intro_audio != 'undefined') {
      cdraw.audio.tags['theme_intro_audio'] = cdraw.audio.createTag(cdraw.theme.data.themes[ cdraw.theme.data.current_theme ].intro_audio);
    }
  ...
  loadTheme: function(theme) {
    ...
    cdraw.theme.setThemeBrowserButtonCaption('loading...');
    ...
    $('#cgp-tool-stamp, #cgp-clipart').stop().css({opacity: 0});
  ...
  themeImageLoaded: function() {
    ...
    if (cdraw.theme.loading_images_left == 0) {
      $('#cgp-tool-stamp, #cgp-clipart').stop().css({opacity: 0}).animate({opacity: 1});
      cdraw.theme.setThemeBrowserButtonCaption(cdraw.theme.data.themes[cdraw.theme.data.current_theme].name)
    }
  },

  setThemeBrowserButtonCaption: function(caption) {
    $('#cgp-btn-theme-browser-caption').html(caption);
    var top = Math.round(($('#cgp-btn-theme-browser').height() - $('#cgp-btn-theme-browser-caption').height()) / 2);
    $('#cgp-btn-theme-browser-caption').css({marginTop: top});
  },
  ...
  validateData: function() {
    ...
    for (i in this.data.themes) {
      if (typeof this.data.themes[i].enabled == 'undefined' || !this.data.themes[i].enabled) {
        delete this.data.themes[i];
      }
    ...
    for (i in this.data.themes) {
      if (typeof this.data.themes[i] == 'object') {
        ...
        if (typeof this.data.themes[i].intro_audio != 'undefined' && !$.isArray(this.data.themes[i].intro_audio)) {
          this.data_errors.push('Invalid value set for "intro_audio" of theme "'+i+'", it should be an array of paths to different formats of the intro audio.');
        }
...
cdraw.initializeNewCanvas = function() {
  ...
  if (cdraw.background.display_frame != cgp_conf.background_tool.initial_frame) {
    cdraw.background.display_frame = cgp_conf.background_tool.initial_frame;
    cdraw.background.setupFrame();
  }

  cdraw.canvas_history.reset();
...
cdraw.setupPlayButton = function() {
  $('#cgp-btn-play').click(function() {
    if (typeof cdraw.audio.tags['theme_intro_audio'] != 'undefined') {
      cdraw.audio.playHintAfter('theme_intro_audio', ['game_title', 'play_game']);
    }
...
cdraw.audio = {
  ...
  play_to_end_tag_ids: ['game_title', 'play_game', 'theme_intro_audio'],
  
  setup: function() {
    for (var id in cgp_conf.audio) {
      if ($.isArray(cgp_conf.audio[id])) {
        var sources = cgp_conf.audio[id];
        for (var i=0; i<sources.length; i++) {
          sources[i] = cgp_conf.audio_base_path + sources[i];
        }
        this.tags[id] = this.createTag(sources);
      }
    }

    this.setupHint('#cgp-btn-help', 'btn_help', true);
    this.setupHint('#cgp-btn-theme-browser', 'btn_theme_browser');
    this.setupHint('#cgp-tool-clipart', 'tool_clipart');
    ...
    this.setupHint('#cgp-eraser-btn-undo', 'btn_undo');
    this.setupHint('#cgp-btn-background-frame', 'background_change_frame');
  },
  ...
  createTag: function(sources) {
    ...
    for (var i=0; i<sources.length; i++) {
      ...
      sources_tags += '<source src="' + sources[i] + '" type="' + type + '" />';
    }
  ...
  playHint: function(tag_id, timeout) {
    if (typeof this.tags[tag_id] == 'undefined' || !this.tags[tag_id]) {
      return;
    }
    ...
    if (timeout) {
      this.timeout = window.setTimeout(function() {play(tag_id);}, timeout);
    }
    else {
      play(tag_id);
    }
  ...
  /* will play a hint only after the audio files specified in after_tags_id have completed playing, and only after all dialogs have been closed (help, done) */
  playHintAfter: function(tag_id, after_tags_id) {
    if ($.inArray(this.currenlty_playing_tag_id, after_tags_id) == -1 && $('#cgp-video-help:visible, #cgp-donedlg:visible').length == 0) {
      this.playHint(tag_id, 0);
      return;
    }

    var play_after_interval = window.setInterval(function() {
      if ($.inArray(cdraw.audio.currenlty_playing_tag_id, after_tags_id) == -1 && $('#cgp-video-help:visible, #cgp-donedlg:visible').length == 0) {
        window.clearInterval(play_after_interval);
        cdraw.audio.playHint(tag_id, 0);
      }
    }, 100);
  },
...
cdraw.brush_tool = {
  ...
  onMouseUp: function(event) {
    ...
    cdraw.canvas_history.push(this.path);
  ...
  fgColorChanged: function() {
    ...
    $('#cgp-tools-brush-drawer-options .size-selector .size-selector-sample li .size-selector-selected-color').css({backgroundColor: this.color});
  },
...
cdraw.eraser_tool = {
  ...
  setup: function() {
    ...
    $('#cgp-eraser-btn-undo').click(function() {cdraw.canvas_history.undo();});
  },
  ...
  onMouseMove: function(event) {
    ...
    if (this.last_checked_point && this.last_checked_point.getDistance(event.point, false) < cdraw.eraser_tool.size * 0.2) {
      return;
    }

    var hitResult = null;
    while (hitResult = cdraw.drawing_layer.hitTest(event.point, {fill: true, stroke: true, segments: true, tolerance: this.size * this.path_tolerance})) {
      hitResult.item.visible = false;
      cdraw.canvas_history.push(hitResult.item);
    }
    ...
    while (i<cdraw.drawing_layer.children.length) {
      if (cdraw.drawing_layer.children[i] instanceof Raster && cdraw.drawing_layer.children[i].visible && cdraw.drawing_layer.children[i].bounds.intersects(eraser_bounds)) {
        var removed_child = cdraw.drawing_layer.children[i];
        cdraw.drawing_layer.children[i].visible = false;
        cdraw.canvas_history.push(cdraw.drawing_layer.children[i]);
      }
      else {
        i++;
      }
...
cdraw.stamp_tool = {
  ...
  onMouseMove: function(event) {
    ...
    cdraw.canvas_history.push(cdraw.drawing_layer.lastChild);
...
cdraw.clipart_tool = {
  ...
  clipartPalletePictureSelected: function() {
    ...
    cdraw.canvas_history.push(raster);
    view.draw();
  },
  ...
  onMouseDown: function(event) {
    if (this.active_raster) {
      if (this.controls.zoomin.bounds.contains(event.point) && this.active_raster.bounds.width < view.size.width - 64 && this.active_raster.bounds.height < view.size.height - 64) {
        ...
        this.active_raster.scale(1.1);
        this.active_raster.custom_image.selection.scale(1.1, this.active_raster.custom_image.selection.bounds.topLeft);
        this.active_raster.position = this.active_raster.custom_image.selection.position.clone();
        this.updateControlsPosition();
        return;
      }
  ...
  onMouseMove: function(event) {
    ...
    if (this.controls.layer.visible) {
      ...
      if (this._toogleButtons(this.controls.zoomin, this.controls.zoomin_hover, event.point)) {
        hovering = true;
        cdraw.audio.playHint('clipart_control_zoom_in');
      }

      if (this._toogleButtons(this.controls.zoomout, this.controls.zoomout_hover, event.point)) {
        hovering = true;
        cdraw.audio.playHint('clipart_control_zoom_out');
      }

      if (this._toogleButtons(this.controls.rotate, this.controls.rotate_hover, event.point)) {
        hovering = true;
        cdraw.audio.playHint('clipart_control_zoom_rotate');
      }

      if (this._toogleButtons(this.controls.remove, this.controls.remove_hover, event.point)) {
        hovering = true;
        cdraw.audio.playHint('clipart_control_zoom_remove');
      }
...
cdraw.background = {
  display_frame: cgp_conf.background_tool.initial_frame,
  ...
  setup: function() {
    ...
    $('#cgp-btn-background-frame').click(function(){cdraw.background.setNextFrame();});
  },
  ...
  setNextFrame: function() {
    ...
    this.display_frame++;
    if (!$('#background-frame-' + this.display_frame).length) {
      this.display_frame = 0;
    }
    this.setupFrame();
  ...
  setupFrame: function() {
    ...
    $('#cgp-canvas-frame')
      .load(function() {
        ...
        var raster = new Raster(tmp_canvas);
        raster.setBounds(new Rectangle(0, 0, view.size.width, view.size.height));
        raster.drawImage(this, new Point(-4, -2));
      ...
      .attr('src', $('#background-frame-' + this.display_frame).attr('src'));
  }
}

cdraw.canvas_history = {
  items: [],
  crt_idx: -1,
  ...
  push: function(item) {
//    if (this.crt_idx != this.items.length) {
//      for (var i=this.crt_idx; i<this.items.length; i++) {
//        this.items[i].remove();
//      }
//      this.items.splice(this.crt_idx, this.items.length - this.crt_idx);
//    }

    this.items.push({item: item, x: item.position.x, y: item.position.y});
    this.crt_idx = this.items.length - 1;
    if (!item.visible) {
      item.position.x = view.size.width - 10;
      item.position.y = view.size.height - 10;
    }
  },
  ...
  undo: function() {
    ...
    this.items[this.crt_idx].item.visible = !this.items[this.crt_idx].item.visible;
    if (this.items[this.crt_idx].item.visible) {
      this.items[this.crt_idx].item.position.x = this.items[this.crt_idx].x;
      this.items[this.crt_idx].item.position.y = this.items[this.crt_idx].y
    }
    else {
      /*
       * if this item is no longer visible then it should be removed, but make sure it is not still referenced in the undo stack
       * (in other words that it is indeed no longer needed)
       */
      var still_referenced = false;
      for (var i=0; i<this.items.length-1; i++) {
        if (this.items[this.crt_idx].item == this.items[i].item) {
      ...
      if (!still_referenced) {
        this.items[this.crt_idx].item.remove();
      }
    }
    this.items.splice(this.crt_idx, 1);
